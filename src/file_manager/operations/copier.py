...
def copy_file(source: Path, destination_dir: Path, policy: DuplicateHandling, dry_run: bool = False) -> tuple[bool, str, Path | None]:
    ...
    if resolved is None:
        app_logger.info("Copy skipped for %s due to policy status=%s", source, status)
        return True, status, None

    if dry_run:
        app_logger.info("Dry run copy %s -> %s", source, resolved)
        return True, f"dry-run-{status}", resolved

    try:
        shutil.copy2(source, resolved)
        app_logger.info("Copied %s -> %s", source, resolved)
        return True, status, resolved
    except Exception as exc:  # noqa: BLE001
        error_logger.exception("Copy failed %s -> %s: %s", source, resolved, exc)
